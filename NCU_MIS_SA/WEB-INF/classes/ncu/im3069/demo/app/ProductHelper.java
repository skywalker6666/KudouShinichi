...
public class ProductHelper {
    ...
    public Product getBySellerId(String idtbl_product) {
        /** 新建一個 Product 物件之 m 變數，用於紀錄每一位查詢回之商品資料 */
        Product p = null;
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `missa`.`tbl_product` WHERE `tbl_product`.`sellerID` = ? LIMIT 1";
    ...
    public JSONObject updateInventory(Product p) {
        /** 紀錄回傳之資料 */
        JSONArray jsa = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `missa`.`tbl_product` SET  `inventory` = ?   WHERE `idtbl_product` = ?";
            /** 取得所需之參數 */                  
            int inventory = p.getInventory();          
            int  idtbl_product= p.getID();
            
            /** 將參數回填至SQL指令當中 */
            pres = conn.prepareStatement(sql);
            pres.setInt(1, inventory);           
            pres.setInt(2, idtbl_product);
            /** 執行更新之SQL指令並記錄影響之行數 */
            row = pres.executeUpdate();
